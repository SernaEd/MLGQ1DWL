...
% Conclusion
    ...
    From the methodological side, the combination of a sparse Schur-complement $LU$ solver for the discretized Cayley step (Eq.~\eqref{eq:sistemaMatricial}) and a Complex Absorbing Potential at the box edges (Eq.~\eqref{eq:CAP}) enables stable long-time evolution of the GWP, while the positive-rightward-flux selection (Eq.~\eqref{eq:rightwardFlux}) yields a clean and robust extraction of the transmission coefficient even for pseudospinor configurations that split into counter-propagating sub-packets.
