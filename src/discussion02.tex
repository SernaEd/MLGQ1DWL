...
\begin{itemize}
    ...
    For pseudospinor configurations that populate both Dirac branches (\textit{e.g.}, $\xi=(1,1)^T$, $(1,i)^T$, $(1,e^{i\pi/4})^T$), the GWP splits into counter-propagating sub-packets; in those cases the incident and transmitted currents reported here are obtained from the positive-rightward-flux selection of Eq.~\eqref{eq:rightwardFlux}, which isolates the physically right-moving component and prevents the left-moving sub-packet from spuriously cancelling the transmitted signal.
